...
// POST /register endpoint: Register a new user
router.post(
  "/register",
  [
  ...
  async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({
        error: "Internal Server Error: Error registering user",
      });
    }
  }
);

// POST /login enpoint: Login a user
router.post(
  "/login",
  [
    check("email", "Please include a valid email").isEmail(),
  ...
  async (req, res) => {
    ...
    try {
      ...
      console.error(err.message);
      res.status(500).json({
        error: "Internal Server Error: Error logging in the user",
      });
    }
  }
);
